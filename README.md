...
#### 1-1. 프로젝트 목적
네이버 웹툰 크롤링을 통해 EDA를 하는 것이 주 목적이며, 가설 설정 후 기술 통계와 시각화를 통해 가설에 대한 결론 도출 및 웹툰의 분포와 장르별 특징을 파악하고, 데이터의 세분화를 통해 깊게 들여다보며 색다른 인사이트를 찾는 것입니다.
...
**가설 1. 순위가 높은 웹툰은 평점과 분량, 참여도 모두 높을 것이다.**
```
- Rank의 파생변수 Score와 참여도간의 상관계수가 0.81로 강한 양의 상관관계를 보임.
- 평점과 분량은 각각 0.051, 0.085로 관계가 없다고 볼 수 있다.

- 카테고리에 대한 데이터 세분화
    - 에피소드 : 평점, 분량이 각각 0.39, -0.38으로 약한 상관관계를 가짐
    - 스토리 : 평점, 분량 모두 Score와 관계 없음
    - 옴니버스 : 참여도, 평점이 각각 0.43, -0.41로 약한 상관관계를 가짐
```
**결론**<br/>
전체적으로 순위가 높을수록 참여도가 높았지만 세분화 시 옴니버스는 참여도에 대해 약한 상관관계를 가졌다. <br/>에피소드와 옴니버스에서 평점과 분량에 대해서도 상관도가 존재했지만 스토리의 비율이 압도적으로 높기에 전반적인 관점에서는 관계가 없다고 판단할 수 있다.

<br/>

**가설 2. 평점이 낮은 회차는 분량과 참여도가 낮을 것이다.**
```
- 평점과 참여도간 상관계수가 -0.4이고 세분화 시에도 약한 음의 상관관계를 보임
```
**결론**<br/>
평점이 낮은 회차일수록 참여도가 높다.

<br/>

**가설 3. 가을에 로맨스 장르의 웹툰이 인기가 많을 것이다.**<br/>
```
결론 : 월별 장르간 웹툰 수, 참여도를 기준으로 바라봤을 때 로맨스, 감성 장르의 웹툰은 봄, 여름에 인가가 많다.
```

<br/>

### 2-2. 기간별, 장르별 웹툰 빈도분석 및 추세분석
<br/>
...
- 1분위수를 기준으로 분류를 했을 때의 통계량과 분포에 대한 차이가 있다고 볼 수 없었다.
...
```
- 가설 3에서 두 가지의 다른 관점으로 데이터를 바라봤을 때 월별 인기 장르가 달랐습니다. 분석목표가 다름에 따라 결과의 차이가 발생할 수 있기에 항상 신중한 판단이 필요하다고 느꼈습니다.

- 세분화 등의 데이터를 다양하게 핸들링하면서 집요하게 바라보는 습관이 중요하다는 것을 알았습니다.

- 분석결과에 대해 어떻게 성과를 낼 수 있을지에 대한 고민이 필요하다고 느꼈습니다.
...
> 연도별 웹툰 크롤링 code : [yearly_webtoon_crawl.py](https://github.com/aeea-0605/eda-repo-3/blob/main/yearly_webtoon_crawl.py)
